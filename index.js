...
class VL53L0X extends I2CCore {
    ...
    async _getMeasurementTimingBudgetInternal (pin) {
    ...
    async _getMeasurementTimingBudget (pin) {
        if (pin) {
    ...
    async _setSignalRateLimit (limit_Mcps, pin) {
    ...
    async _getRangeMillimeters (pin) {
        const toReturn = {}
        ...
        if (pin) {
            if (this._addresses[pin]) {
                ...
                throw new Error(`Invalid pin number. Available Pins: [${Object.keys(this._addresses).map((pin) => " '" + pin + "' ")}]`)
            }
        } else {
            for (const p of Object.keys(this._addresses)) {
                // I guess we need to make sure the stop variable didn't change somehow...
    ...
    async _performSingleRefCalibrationInternal (vhv_init_byte, pin) {
    ...
    async _performSingleRefCalibration (vhv_init_byte, pin) {
        if (pin) {
    ...
    async _setVcselPulsePeriod (type, period_pclks, pin) {
    ...
    async _getVcselPulsePeriod (type, pin) {
        if (pin) {
